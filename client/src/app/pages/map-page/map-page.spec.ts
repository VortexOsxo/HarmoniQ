...
// Stub template exposes the toggle/close buttons and the sources panel so we can
// test the component's logic through the DOM without rendering complex child components.
const STUB_TEMPLATE = `
...
async function renderComponent() {
  TestBed.overrideComponent(MapPage, { set: { template: STUB_TEMPLATE, imports: [CommonModule] } });
...
describe('MapPage', () => {
  ...
  describe('tutorial subscription', () => {
    it('should close sources panel when tutorial becomes active with welcome screen', async () => {
      ...
      await waitFor(() => expect(screen.queryByTestId('sources-panel')).not.toBeInTheDocument());
    });
